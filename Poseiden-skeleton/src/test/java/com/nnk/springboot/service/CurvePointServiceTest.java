...
class CurvePointServiceTest {
    ...
    void findAll_returnsList() {
        when(repo.findAll()).thenReturn(List.of(
                new CurvePoint(new BigDecimal("1.00"), new BigDecimal("10.00")),
                new CurvePoint(new BigDecimal("2.00"), new BigDecimal("20.00"))
        ));
    ...
    void findById_existing_returnsValue() {
        CurvePoint cp = new CurvePoint(new BigDecimal("1.00"), new BigDecimal("10.00"));
        cp.setId(5);
    ...
    void save_persistsEntity() {
        CurvePoint cp = new CurvePoint(new BigDecimal("3.00"), new BigDecimal("30.00"));
    ...
    void update_existing_updatesFields() {
        CurvePoint existing = new CurvePoint(new BigDecimal("1.00"), new BigDecimal("10.00"));
        existing.setId(1);
        when(repo.findById(1)).thenReturn(Optional.of(existing));
        when(repo.save(any(CurvePoint.class))).thenAnswer(inv -> inv.getArgument(0));

        CurvePoint changes = new CurvePoint(new BigDecimal("9.00"), new BigDecimal("99.00"));
